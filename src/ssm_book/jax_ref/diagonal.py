r"""JAX port of the diagonal state space model (book section 3.5).

Mirrors :mod:`ssm_book.numpy_ref.diagonal`.
"""
from __future__ import annotations

import jax.numpy as jnp

# Import side effect: ssm_book.jax_ref.__init__ enables 64-bit precision.
...
def diagonal_kernel(lambdas, weights, L):
    ...
    m = jnp.arange(L)
    V = lambdas[:, None] ** m[None, :]  # V_{nm} = lambda_n ** m
    return weights @ V
...
def s4d_lin_init(M):
    ...
    n = jnp.arange(M)
    return -0.5 + 1j * jnp.pi * n


def s4d_inv_init(M):
    r"""S4D-Inv eigenvalues, ``n=0..M-1`` (book section 3.5.6)."""
    n = jnp.arange(M)
    return -0.5 + 1j * (M / jnp.pi) * (M / (2 * n + 1) - 1)
